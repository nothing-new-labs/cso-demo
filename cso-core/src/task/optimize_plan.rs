...
impl<T: OptimizerType> From<OptimizePlanTask> for Task<T> {
    #[inline]
...
impl OptimizePlanTask {
    ...
    fn filter_invalid_rules<T: OptimizerType>(
        plan: &GroupPlan,
        candidate_rules: &[RuleRef<T>],
        valid_rules: &mut Vec<RuleRef<T>>,
    ) {
        candidate_rules
            .iter()
    ...
    fn get_rules<T: OptimizerType>(&self, rule_set: &RuleSet<T>) -> Vec<RuleRef<T>> {
